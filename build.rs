...
fn main() {
    ...
    for sq in 0..64 {
        init_magic(sq, PieceType::Rook, is_bmi2, &mut rooks, &mut table);
        init_magic(sq, PieceType::Bishop, is_bmi2, &mut bishops, &mut table);
    }

    writeln!(f, "// Auto-generated by build.rs").unwrap();
    writeln!(f).unwrap();

    // BMI2 extracts the occupancy with PEXT, so the magic and its shift are dead
    // weight there and the entry is two fields instead of four.
    for (name, entries) in [("ROOKS", &rooks), ("BISHOPS", &bishops)] {
        writeln!(f, "pub static {name}: [MagicEntry; 64] = [").unwrap();
    ...
    for (name, rows) in [("LINES", lines), ("BETWEEN", between)] {
        writeln!(f, "pub static {name}: [[u64; 64]; 64] = [").unwrap();
        for row in rows {
            write!(f, "    [").unwrap();
            for val in row {
                write!(f, "{val:#018X}, ").unwrap();
...
fn init_magic(square: Square, pt: PieceType, is_bmi2: bool, entries: &mut Vec<MagicEntry>, table: &mut Vec<u64>) {
    let mask = match pt {
