...
**Direct (`eval_linear_grad`): the reference.** Evaluate the position, read each feature coefficient straight off the board, phase, and openness, scatter `outer_deriv · coefficient` into the gradient vector. ~90 f64 ops on top of the eval itself. It has one caller, the oracle test; what runs every epoch is the cached twin below.
