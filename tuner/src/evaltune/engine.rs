...
//!
//! Every other module here imports from this file, so the surface between the two is
//! one list and cannot grow without someone editing it.
//!
//! `tape.rs` is the exception. The oracle checks Soul's cached gradient path against
//! its dual path, which is the engine testing itself rather than the tuner asking it
//! for anything, and re-exporting `autograd` and `combiner` here would make the
//! surface look twice as wide as it is.
...
#[cfg(test)]
mod tests {
    /// `tape.rs` is the stated exception; anything else naming `soul::` has gone
    /// around the door.
    #[test]
    fn nothing_else_reaches_past_this_file() {
        ...
        for entry in std::fs::read_dir(dir).expect("the module's own directory") {
            ...
            if name == "engine.rs" || name == "tape.rs" || path.extension().is_none_or(|e| e != "rs") {
                continue;
            }
