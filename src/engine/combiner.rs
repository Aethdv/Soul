...
/// Buckets the evaluation terms fill, in two shapes:
/// - Pre-tapered (`mg_eg`, `mobility`): blended inside the SIMD lanes to stay vectorized.
/// - Raw (`bonus_mg`/`bonus_eg`, `safety_us`, `safety_them`, `xray`): tapered once here, so
///   per-term rounding cannot accumulate.
pub struct Accumulators<T: EvalMath> {
    ...
    pub danger_us: T,
    pub danger_them: T,
    /// X-ray and king-ring attack differential.
    pub xray: T,
}
...
/// The one non-linearity in the eval, so pressure accelerates instead of scaling with the weight
/// table alone. Curvature is a numerator so that `c = 0` is exactly the linear block; as a divisor
/// it could only approach it.
#[inline(always)]
...
/// The one rounding site in the combiner. Truncation is toward zero, so a negative score rounds
/// up, and `eg = 0` degenerates to the middlegame-only taper the king-safety block wants.
#[inline(always)]
...
impl Combiner for LinearCombiner {
    ...
    fn backward(
        ...
        let king_safety = KingSafetyUpstream {
            shelter: d_safety,
            danger_us: -d_safety * slope(buckets.danger_us),
            danger_them: d_safety * slope(buckets.danger_them),
        };

        // ∂L/∂c = (∂L/∂safety) · (∂safety/∂c) = d_safety · (p_them² - p_us²) / scale
        let (us, them) = (buckets.danger_us, buckets.danger_them);
        grads[LAYOUT.king_danger_offset] += d_safety * (them * them - us * us) / scale;

        BucketUpstreams { mg_eg: taper_pair, mobility: taper_pair, bonus: taper_pair, king_safety, xray: d_safety }
    }
}
